...
public class TimeOffRequestsDAO extends SQL {
	...
	// INSERT INTO `timeOffRequest` (`id`, `employee_ID`, `startDate`, `endDate`,
	// `type`, `requestStatus`) VALUES (NULL, '', '', '', '', '')
	...
	public static TimeOffRequest setTimeOffRequest(Employee currentEmployee, String startDate, String endDate,
			String type, int requestStatus) {

		try {
			ps = conn.prepareStatement(
					"INSERT INTO timeOffRequest (employee_ID, startDate, endDate, type, requestStatus) VALUES (?, ?, ?, ?, ?)");
	...
	public static ObservableList<TimeOffRequest> getTimeOffRequestByEmployeeID(Employee e) {
		...
		String type;
		int requestStatus;
		ObservableList<TimeOffRequest> dayOffRequests = FXCollections.observableArrayList();
	...
	public static ObservableList<TimeOffRequest> getAllTimeOffRequests() {
		...
		try {
			...
			while (rs.next()) { // Position the cursor 4
				...
				dayOffRequests.add(new TimeOffRequest(id,
						new Employee(id1, firstName, lastName, accruedTime, isAdmin, employeeType), startDate, endDate,
						type, requestStatus));
